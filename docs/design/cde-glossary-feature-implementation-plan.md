...
- F02 đọc Data Dictionary/CDE Approved trong active scope và Archived trong historical scope theo quyền; không triển khai mutation/cutover. Consumer-only mặc định vẫn mở active Approved mới nhất nhưng selector/history cho phép chọn các Data Dictionary Archived và đọc published CDE snapshots trong frozen manifest; không thấy working/non-Approved.
...
- Bất kỳ lỗi nào trước commit, bao gồm lỗi insert snapshot, update head, insert outbox hoặc delete working, phải rollback toàn bộ. Không được tồn tại snapshot mồ côi, head trỏ sai, outbox thiếu, working bị xóa sớm hoặc lifecycle/search side effect phản ánh dữ liệu chưa commit.
...
- Search projection phục vụ global discovery/Assets và external event sử dụng transactional outbox, có eventual consistency. Lỗi xử lý outbox sau commit không biến Approve đã commit thành thất bại, không xóa snapshot và không rollback head; event giữ trạng thái pending/error để retry idempotent. Xử lý lặp cùng event không được tạo document hoặc event nghiệp vụ trùng.
- Consumer list/search/filter CDE đọc database và phải thấy snapshot vừa commit ngay. Không fallback sang native/custom search index; global discovery có thể trễ nhưng không được dùng để quyết định publication đã thành công hay chưa.
...
- Failure injection sau từng boundary insert snapshot, update head, insert outbox và delete working chứng minh transaction rollback toàn bộ; published API, F11/F12 và downstream search projection không quan sát dữ liệu thất bại.
...
- Outbox test bao phủ global-discovery index/event xử lý thành công, lỗi để event pending/error, retry thành công và xử lý lặp idempotent. Published detail/history và Consumer F11/F12 vẫn đọc được ngay từ database khi search projection đang lỗi; không lộ working payload.
...
- Bất kỳ lỗi nào trước commit phải rollback toàn bộ. Cập nhật search projection phục vụ global discovery chỉ chạy sau commit; lỗi index sau commit không được xóa working version đã tạo hoặc biến kết quả đã commit thành thất bại.
...
- Bất kỳ lỗi nào trước commit phải rollback toàn bộ; không thay đổi identity, snapshot, published head, hash, outbox, CDE membership hoặc native entity. Cập nhật search projection phục vụ global discovery chỉ chạy sau commit; lỗi index sau commit được log/retry và không đảo kết quả tạo working đã commit.
...
- Failure injection tại các boundary sau lock, validation, dựng payload và insert chứng minh rollback toàn bộ. Lỗi search projection sau commit không đổi HTTP success hoặc xóa working đã tạo.
...
- Consumer-only nhận published `Approved` rows của Data Dictionary active và được đọc archived published rows thuộc frozen manifest khi chọn Data Dictionary Archived. Consumer truy cập working scope nhận `404`; archived luôn read-only và không lộ predecessor non-Approved/working rows. Manager nhận published và working rows đúng scope theo capability hiệu lực. Không suy quyền từ tên role.
...
- F11 và F12 cùng đọc một database-backed flat read model authoritative. F12 chỉ bổ sung search/filter predicates vào query của F11; việc bật một criteria không được chuyển nguồn dữ liệu, consistency model hoặc authorization model. Native index `glossaryTerm` và mọi custom search index không được dùng để quyết định row/status/total của bảng CDE.
...
- Sau khi scope `1` được archive, Consumer vẫn chọn được Data Dictionary `1` và flat-list chỉ trả archived published `1.0`, `1.1` từ frozen manifest; không trả predecessor working/non-Approved hoặc row `2.x`.
...
- F12 dùng `GET /v1/glossaryTerms/search` với `glossary`, canonical `parentBusinessVersion`, `q`, `statuses`, `domainIds`, `ownerIds`, `dataSourceTags`, `classificationTags`, `limit` và `offset`. Với Data Dictionary, endpoint bắt buộc dùng cùng database query service/read model của F11. Caller Native/DQ không có parent scope giữ nguyên behavior hiện hữu.
- Không tạo `cdeBusinessVersion`, `cdeBusinessVersionPublished`, alias theo role hoặc authorization projection trong search engine. Row identity vẫn là `(termId, parentBusinessVersion,businessVersion)` tại database/read DTO; nhiều business version của cùng term không được ghi đè hoặc collapse.
- Database query lấy đủ row DTO trực tiếp từ business snapshot/working stores và archive manifest. Filter dùng UUID/FQN ổn định, không dùng domain display name, owner name hoặc label có thể đổi/trùng. Quan hệ nhiều-nhiều cần bảng nối và index phù hợp; không scan/deserialize toàn bộ payload rồi lọc trong application.
- Search `q` là text literal sau trim/Unicode normalization, tối đa 200 ký tự, tìm đồng thời `name OR displayName`; client không được truyền SQL fragment, search-engine DSL hoặc wildcard expression. Backend dùng bind parameter và escape wildcard của `LIKE`/`ILIKE` (hoặc cột normalized tương đương) để mọi ký tự người dùng là literal. Frontend debounce 500 ms và nút clear đưa bảng về F11 default list.
- `statuses` hỗ trợ `Draft`, `In Review`, `Rejected`, `Approved`; `Archived` chỉ hợp lệ khi parent scope là Archived. Nhiều giá trị trong cùng một multi-select kết hợp `OR`; các nhóm search/status/domain/owner/tag kết hợp `AND`. Parameter rỗng sau trim được coi là absent; UUID/FQN/status sai, CSV có phần tử rỗng hoặc vượt allowlist/giới hạn trả `400`.
- Thứ tự xử lý bắt buộc là validate request → resolve/authorize Dictionary scope từ database → dựng effective allowed-row restriction → dựng flat rows đúng scope → áp authorization/search/filters → tính `total` → stable sort → pagination. Tất cả bước từ allowed-row restriction đến pagination phải nằm trong database query/bounded query plan; không page identity trước rồi hydrate hoặc post-filter trong application. Consumer-only luôn bị khóa ở published Approved rows của active Dictionary hoặc frozen archived manifest; gửi `Draft`/`Rejected` không được mở rộng kết quả. Working/unauthorized parent scope vẫn trả `404` như F11.
- Query phải khóa cả `glossaryId`, canonical `parentBusinessVersion` và loại scope đã resolve. Index database tối thiểu bao phủ scope/status/version/order; các bảng nối domain/owner/tag có index ở khóa row và giá trị filter. Dùng `EXISTS`/join có kiểm soát để không nhân bản row hoặc làm sai `COUNT`; execution plan phải được xác nhận trên PostgreSQL và MySQL.
- F11 và F12 trả cùng row DTO `{ data, paging: { total, limit, offset } }`, cùng page sizes `10/15/25/50`, stable default order và strong read-after-commit semantics. Sau mọi workflow/import commit, list không filter và search/filter phải quan sát cùng trạng thái authoritative; F12 không phụ thuộc outbox, retry, reconciliation hoặc reindex.
...
- Fixture cùng `termId A`, parent scope `1` có `1.0`, `1.1`, `1.2` tạo ba database rows/DTO rows độc lập; không row nào collapse row khác. Identity B cùng mã ở scope `2` không lọt vào query scope `1`.
- Consumer database query chỉ trả Approved rows của active scope hoặc frozen archived published rows đúng manifest. Gửi `Draft` hoặc `Draft,Approved` không làm lộ working/non-Approved qua data, `total`, sort hoặc empty state. Manager/partial Manager chỉ nhận row theo effective capability và parent scope.
- Pagination test bao phủ page đầu/cuối/out-of-range và không trùng/mất row trên database snapshot cố định theo stable default order.
- Read-after-write test bao phủ create/update/status transition/approve/cutover/archive/delete/import: F11 và F12 quan sát cùng committed state ngay lập tức, không cần refresh index, xử lý outbox hoặc reconciliation.
- F11-vs-F12 parity test trên cùng committed database state chứng minh cùng authorization, row DTO, default order và total khi F12 không có content filter. Native/DQ Glossary giữ nguyên contract hiện hữu khi không có `parentBusinessVersion`.
...
- Có query-count/performance test trên dataset business-version đủ lớn; authorization và hydration không N+1. Dùng execution-plan regression cho tổ hợp search/status/domain/owner/tag, kiểm tra index coverage, stable pagination và parity PostgreSQL/MySQL. Test read-after-write chứng minh F11/F12 cùng thấy ngay trạng thái vừa commit.
...
- Backend resolve và authorize scope authoritative giống F11. Scope không tồn tại hoặc actor không có quyền xem trả `404`. Consumer-only export published `Approved` rows của Data Dictionary active hoặc archived published rows thuộc frozen manifest khi xem Data Dictionary Archived; Consumer không được export working/non-Approved. Người có quyền working export published và working rows đúng scope theo capability hiệu lực.
...
- Nếu CDE chưa tồn tại trong scope thì luôn tạo identity và Draft `N.0` theo F03. Với `SKIP_EXISTING`, mọi row match CDE đã tồn tại được đánh dấu `SKIP`, không validate các quyền mutation/reference chỉ phục vụ update, không tăng revision, không transition, không audit mutation và không phát outbox. Với `OVERWRITE_EXISTING`, working `Draft`, `InReview` hoặc `Rejected` được thay thế toàn bộ business payload; `InReview` và `Rejected` đồng thời được chuyển về `Draft`, vô hiệu hóa phiên duyệt cũ nhưng giữ lịch sử transition/reject trong audit. Nếu chỉ có `Approved` và chưa có working thì tạo business version Draft kế tiếp theo F06 rồi ghi payload. Mọi row được tạo/cập nhật thành công đều là `Draft`; không tự submit hoặc approve.
- Import dùng full-state semantics: với create, ô optional trống không tạo key trong `extension`/tạo collection rỗng tương ứng; với update, `extension` được thay toàn bộ nên key custom property bị bỏ khỏi map có nghĩa là xóa giá trị hiện tại. Không gửi JSON `null` cho custom property typed vì validator không chấp nhận `null` thay cho string/array. `name` và mọi field server-owned không được thay đổi. Hai cột ngày chỉ nhận chính xác `dd/MM/yyyy`, backend parse nghiêm ngặt rồi chuẩn hóa về ISO string cho lưu trữ nội bộ; `expirationDate` không trước `effectiveDate`. Quy định chất lượng chỉ nhận `Có/Không` và normalize thành JSON array enum một phần tử `["Y"]`/`["N"]`; ô trống không tạo key.
- Reference trong file dùng `displayName` giống file Export; nhiều giá trị được phân cách bằng line break. Backend chuẩn hóa Unicode/case rồi resolve `displayName` trong đúng loại entity và chỉ chấp nhận khi có đúng một kết quả. Không fallback sang FQN/name do người dùng nhập, không tự tạo owner/domain/tag; reference không tồn tại, trùng display name, sai loại hoặc trái quyền là row error. Chủ sở hữu được resolve duy nhất trên tập hợp User + Team; tag phải resolve duy nhất trong đúng classification `DataSource`, `DataClassification` hoặc `PersonalData` tương ứng. Import không có reviewer.
- Catalog dùng để resolve `displayName` được tải tối đa một lần cho mỗi entity type trong một preview. Repository hỗ trợ soft-delete dùng `NON_DELETED`; repository không có cột/capability soft-delete bắt buộc dùng `ALL`, không được sinh predicate `deleted`. Exception SQL/JDBI/stack trace không được trả trong row error: backend log chi tiết kèm row number và trả mã ổn định `REFERENCE_LOOKUP_FAILED` với message an toàn.
- Luồng bắt buộc gồm preview rồi commit. `POST /v1/glossaryTerms/import/preview?glossary={glossaryId}&parentBusinessVersion={N}&existingCodePolicy={SKIP_EXISTING|OVERWRITE_EXISTING}` nhận multipart workbook, parse/validate nhưng tuyệt đối không mutation/outbox, và tạo import session server-side bind với actor, scope, policy, SHA-256 file, normalized plan, parent state cùng expected revision của từng row sẽ mutation; session có TTL 30 phút và chỉ commit một lần. Response trả `importSessionId`, expiry, policy, summary create/create-version/update/skip/warning/error, kết quả từng dòng và `canCommit`. Thiếu hoặc sai policy trả `400`; UI mặc định `SKIP_EXISTING` nhưng luôn gửi tường minh.
- Preview bắt buộc chạy cùng custom-property schema validator mà commit/`prepareInternal` sử dụng trên normalized `extension`. Sai kiểu string/array/date/enum phải trở thành row error và `canCommit=false`; không được để preview thành công rồi mới phát hiện lỗi schema tại commit.
- `POST /v1/glossaryTerms/import/{importSessionId}/commit` không nhận lại payload/file. Trước khi ghi, backend re-authorize actor và kiểm tra session còn hạn/chưa dùng, parent chưa cutover/archive, mọi expected revision/state/unique key/reference vẫn đúng. Stale preview, concurrent create/update, permission change hoặc scope change trả `409`, không tự merge/retry/refresh và không ghi một phần.
- Commit nguyên tử đối với toàn bộ tập row có action mutation trong một database transaction; các row `SKIP` là kết quả đã hoạch định từ preview, không phải partial success. F14 không cho phép bỏ qua row lỗi. Lock parent và working rows theo thứ tự ổn định, dùng service/invariant F03/F04/F06 thay vì native CSV/direct `GlossaryTerm` PATCH. Mỗi working row bị thay đổi tăng `workingRevision` đúng một lần; Data Dictionary revision không đổi. Lỗi ở bất kỳ row mutation nào rollback identity, working record, transition, audit-success và outbox của toàn file.
- Tiến độ commit tái sử dụng hạ tầng WebSocket của OpenMetadata nhưng dùng channel riêng `cdeImportChannel`; `importSessionId` là job key. Backend phát `STARTED`, rồi `IN_PROGRESS` theo batch với `progress=rowsProcessed`, `total=mutationRows`, và chỉ phát `COMPLETED` sau khi transaction commit thành công. Giá trị trong lúc chạy có nghĩa là **đã xử lý**, không phải đã lưu; UI giới hạn tối đa 99% trước commit. Khi rollback, backend phát `FAILED`, không phát `COMPLETED`. Lỗi phát notification chỉ được log và tuyệt đối không làm rollback transaction nghiệp vụ.
- Backend expose capability `canImportCdeDrafts` từ policy hiệu lực, ownership và scope; không hard-code role. Mặc định Admin có quyền, Data Proposer theo policy, Data Steward/Reviewer/Consumer-only không có. Commit re-authorize quyền create/edit/create-version/reopen tương ứng từng row; thiếu quyền ở một row làm fail toàn file. Scope không tồn tại/không được xem trả `404` mà không lộ payload.
- Sau commit, F11 và F12 cùng đọc database authoritative nên phản ánh ngay kết quả import; outbox (nếu có cho integration/event ngoài bảng) không nằm trên critical path của list/search/filter. Audit ghi actor, session, file name đã sanitize/hash, scope, số create/create-version/update, các transition về Draft, thời điểm và kết quả nhưng không ghi workbook/cell content vào log.
- UI hiển thị Import trên Data Dictionary `Approved` active và `Draft` khi có `canImportCdeDrafts`. Action phải điều hướng tới trang import riêng theo route chuẩn `getEntityImportPath(EntityType.GLOSSARY, glossaryFqn)` và truyền `parentBusinessVersion` của Dictionary đang mở trên query string; không mở modal import. Trang import hiển thị hai radio `Bỏ qua bản ghi trùng` (mặc định) và `Cập nhật ghi đè bản ghi`, rồi thực hiện: tải template/chọn file và chọn policy → preview → bảng action/lỗi theo dòng-cột cùng summary → xác nhận commit. Preview có `InReview` dưới policy ghi đè phải cảnh báo rõ các phiên duyệt sẽ bị hủy; dưới policy bỏ qua, các row này chỉ mang action `SKIP`. Khi commit, UI subscribe `cdeImportChannel`, tính `round(progress/total*100)`, hiển thị nội dung “đã xử lý X/Y” và giữ tối đa 99% cho tới event/HTTP success xác nhận commit. Disable/deduplicate request khi loading; chỉ enable commit khi không có error. Thành công điều hướng/reload F11 authoritative; `409`/expired session giữ báo cáo nhưng khóa commit và yêu cầu preview lại, không tự retry.

**Test/DoD**

- Contract test template/preview/commit, hai giá trị `existingCodePolicy`, policy thiếu/sai, policy binding không thể đổi khi commit, content type, scope/version canonical, session TTL/single-use/actor binding và status `200`, `400`, `404`, `409`, `413`; preview có lỗi dữ liệu trả `200` với `canCommit=false`, và native Glossary CSV importer vẫn không được dùng để ghi business data.
- Test cả active `N` và Dictionary Draft `N+1`: cả hai policy vẫn create `N.0` cho mã mới; `SKIP_EXISTING` bỏ qua Draft/InReview/Rejected/Approved mà không mutation; `OVERWRITE_EXISTING` update Draft, thay payload `InReview`/`Rejected` rồi chuyển về Draft và tạo version kế tiếp từ Approved. Không sửa snapshot/parent revision và không reuse identity cùng mã ở scope khác.
- Test full-state/absent-key semantics, 13 header đúng thứ tự, Unicode tiếng Việt, multiline/Markdown, ngày hợp lệ đúng `dd/MM/yyyy`, ngày ISO hoặc ngày lịch không hợp lệ bị từ chối, `Có/Không` tạo đúng `["Y"]`/`["N"]`; ô trống xóa custom property bằng cách bỏ key và không gửi JSON `null`. Preview phải chạy schema validation và chặn mọi lỗi kiểu custom property trước commit. Test duplicate sau normalization, required/unknown/oversized value và resolve reference bằng `displayName`: unique thành công, missing/ambiguous/wrong-classification/unauthorized thất bại. Test cả repository có/không hỗ trợ soft-delete, chứng minh Domain lookup không tham chiếu cột `deleted`; lỗi hạ tầng chỉ lộ `REFERENCE_LOOKUP_FAILED`, không lộ SQL, class exception, statement hoặc stack trace.
- Preview không tạo identity/working/transition/outbox/audit success. Test conflict do working revision, concurrent create, reference/permission thay đổi, session hết hạn hoặc Dictionary cutover/archive giữa preview và commit; tất cả trả `409` và không ghi một phần.
- Inject failure ở row cuối chứng minh rollback toàn file; success tăng mỗi working revision đúng một lần, phát outbox idempotent sau commit nếu integration khác cần, audit đúng summary và reload F11/F12 đều thấy dữ liệu ngay.
- Security/performance test giới hạn 5 MB/5.000 rows/32.000 ký tự, macro/formula/external link/embedded object, malformed XLSX/zip bomb, temp/session quota và cleanup; không log nội dung workbook.
- Frontend test action theo capability/state, điều hướng đúng trang import kèm `parentBusinessVersion` và không mở modal, mặc định/chuyển đổi hai radio policy, policy được gửi tường minh khi preview, đổi policy buộc preview lại, summary/action `SKIP`, cảnh báo hủy `InReview` chỉ khi ghi đè, lỗi theo dòng/cột, disable/double-submit, WebSocket job filtering/progress/cleanup, không vượt 99% trước commit, `FAILED`/rollback, expired session/stale response/`409`, success lên 100% và reload F11, không tự retry.
- UAT bằng template thực tế bao phủ cả active và Draft Dictionary; lỗi dùng mã ổn định và chỉ rõ dòng, cột, giá trị an toàn cùng hướng xử lý.
