...
- Consumer được route vào published projection: active scope chỉ nhận Approved rows; archived scope chỉ nhận frozen archived published rows đúng manifest. Gửi `Draft` hoặc `Draft,Approved` không làm lộ working/non-Approved qua data, `total`, sort hoặc facet. Manager/partial Manager chỉ nhận row theo effective capability và parent scope.
...
- Sau commit, transactional outbox idempotent dùng khóa `importSessionId + rowNumber` cập nhật read/search model; F11 database authoritative phản ánh ngay, F12 có thể eventual consistency. Audit ghi actor, session, file name đã sanitize/hash, scope, số create/create-version/update, các transition về Draft, thời điểm và kết quả nhưng không ghi workbook/cell content vào log.
...
- Inject failure ở row cuối chứng minh rollback toàn file; success tăng mỗi working revision đúng một lần, phát outbox idempotent sau commit, audit đúng summary và reload F11 thấy dữ liệu ngay cả khi F12 còn lag.
